...
transformed data {
  ...
  Q_ast2 = qr_Q(X)[, 1:k] * sqrt(n - 1);
  R_ast2 = qr_R(X)[1:k, ] / sqrt(n - 1);
...
parameters {
  ...
  vector[k] theta2; // coefficients on Q_ast
}
transformed parameters{
  ...
  rho_1 = eta0*c_phi2 + W*eta1*c_phi2 + phi1*inv_sqrt(tau1) 
          + nought[1] + Q_ast1 * theta1;
  rho_2 = nought[2] + M_W*(c_phi2 + Q_ast2 * theta2);
}
model {
  phi2 ~ sparse_car(alpha2, W_sparse, D_sparse, lambda, n, W_n);
...
generated quantities {
  ...
  //   // likelihood of the current parameter values (given the original data)
    log_lik1[i] = neg_binomial_2_lpmf(y1[i] | exp(rho_1[i] + log_offset1[i]), v_sig1);
  ...
  //   // compute likelihood of the current parameter values (given the new data)
    log_lik_rep1[i] = neg_binomial_2_lpmf(yrep1[i] | exp(rho_1[i] + log_offset1[i]),
                                                    v_sig1);
  }
  for (i in 1:m) {
  //   // likelihood of the current parameter values (given the original data)
    log_lik2[i] = neg_binomial_2_lpmf(y2[i] | exp(rho_2[i] + log_offset2[i]), v_sig2);
  //   // generate new data based on current parameter values
    yrep2[i] = neg_binomial_2_rng(exp(rho_2[i] + log_offset2[i]), v_sig2);
      //   // compute likelihood of the current parameter values (given the new data)
    log_lik_rep2[i] = neg_binomial_2_lpmf(yrep2[i] | exp(rho_2[i] + log_offset2[i]),
                                                    v_sig2);
    }
